...
function runCli(file: string, args: string[] = []): { stdout: string; stderr: string; status: number } {
...
function withTempFile(content: string, ext: string, fn: (path: string) => void): void {
...
describe("CLI --help flag", () => {
  it("shows help and exits 0", () => {
    ...
    assert.ok(
      result.stdout.includes("pii-scan") || result.stdout.includes("Usage"),
