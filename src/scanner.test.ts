...
describe("scanContent — CSV", () => {
  it("detects email address in content", () => {
    ...
    assert.ok(
      result.findings.some((f) => f.patternName === "Email Address"),
      "Expected Email Address finding"
    );
  ...
  it("returns LOW risk for synthetic-safe data", () => {
    ...
    assert.equal(result.findings.filter((f) => f.source === "content").length, 0,
      "Expected no content findings");
  ...
  it("detects SSN pattern", () => {
    ...
    assert.ok(
      result.findings.some((f) => f.patternName.includes("SSN")),
  ...
  it("detects column name heuristics for name columns", () => {
    ...
    assert.ok(
      result.findings.some((f) => f.source === "column_name" && (f.column === "first_name" || f.column === "last_name")),
      "Expected column name finding for first_name or last_name"
    );
  });
  ...
  it("detects credit card numbers", () => {
    ...
    assert.ok(
      result.findings.some((f) => f.patternName.includes("Card")),
      "Expected credit card finding"
    );
    assert.equal(result.overallRisk, "HIGH");
  });
  ...
  it("detects US phone numbers", () => {
    ...
    assert.ok(
      result.findings.some((f) => f.patternName === "US Phone Number" || f.source === "column_name"),
      "Expected phone finding"
    );
  });
...
describe("scanContent — JSON", () => {
  ...
  it("handles JSON with data wrapper", () => {
    ...
    assert.ok(
      result.findings.some((f) => f.patternName === "Email Address" || f.source === "column_name"),
      "Expected finding in wrapped JSON"
    );
  });
...
describe("scanColumns", () => {
  it("detects phone numbers in content", () => {
    ...
    assert.ok(
      result.findings.some((f) => f.patternName === "US Phone Number"),
      "Expected US Phone Number finding"
    );
  });
  ...
  it("detects routing numbers in content", () => {
    ...
    assert.ok(
      result.findings.some((f) => f.patternName.includes("Routing") || f.patternName.includes("ABA")),
      "Expected bank routing number finding"
    );
  });
  ...
  it("flags column names matching suspicious patterns", () => {
    ...
    assert.ok(
      result.findings.some((f) => f.source === "column_name"),
      "Expected column name finding for health data"
    );
  });
