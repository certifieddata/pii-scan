...
const result = scanContent(content, absPath);
if (jsonOutput) {
...
for (const f of result.findings) {
    if (!byColumn.has(f.column))
        byColumn.set(f.column, []);
...
for (const [column, findings] of sorted) {
    const colRisk = findings.reduce((acc, f) => (f.risk === "HIGH" || (acc !== "HIGH" && f.risk === "MEDIUM") ? f.risk : acc), "LOW");
